...
from util.error_reporter import error_reporter as er
from util.time_manager import time_manager as tm
...
class RoofInspection(Skill):
    ...
    def get_onscreen_controls(self, api):
        ...
        if self.paused:
            ...
            detail = "Waypoint {}/{}".format(
                self.current_waypoint_index, len(self.global_waypoints)
            )
    ...
    def update(self, api):
        ...
        if self.publish_downsampler.ready(api.utime):
            ...
            # Publish json status.
            status = dict(
                paused=self.paused, status_code=self.status_code.name, **self.status_message
            )
        ...
        if self.global_waypoints:
            ...
            for i, waypoint_id in enumerate(self.global_waypoints):
                ...
                if waypoint is not None:
                    nav_T_center = Transform(waypoint.rotation(), waypoint * np.array([2.0, 0, 0]))
    ...
    def create_mission(
        ...
        global_waypoints = [
            api.waypoints.save_nav_location(
                nav_T_vehicle * np.array([lookat_range, 0, 0]),
        ...
        waypoints = [
            list(api.waypoints.nav_to_gps(nav_T_vehicle.translation()))
