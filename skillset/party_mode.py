...
import util.error_reporter.error_reporter as er
import util.time_manager.time_manager as tm
from vehicle.skills.base_skill import Skill
from vehicle.skills.util.filters import LowPassFilter
...
class PartyMode(Skill):
    ...
    def find_and_lock_subject(self, api):
        ...
        for track in tracker_state.tracks:
            ...
            if (
                track.is_locked
                and (distance < self.MAX_LOCK_DISTANCE)
                and been_long_enough
                and (distance_from_start < self.get_value_for_user_setting("radius"))
            ):
        ...
        if locked_tracks:
            ...
            er.REPORT_QUIET(
                "Found {} locked tracks, chose track {}", len(locked_tracks), chosen_track.track_id
            )
    ...
    def get_return_desired_point_nav(self, vehicle_position):
        ...
        if distance_to_home > self.MAX_DESIRED_POSITION_DISTANCE:
            to_home_vector = (
                to_home_vector / distance_to_home
    ...
    def update(self, api):
        ...
        if self.state is self.STATE.SEARCH:
            ...
            if (not self.subject_has_moved) and (subject_speed_filtered > self.MOVING_CUTOFF_SPEED):
                self.subject_has_moved = True

            er.REPORT_QUIET(
                "Tracking: {}s, Has moved: {}, Filtered Speed: {}",
                locked_time,
                self.subject_has_moved,
                subject_speed_filtered,
            )
